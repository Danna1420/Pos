...
//        m_keyvalue = DocumentLoaderBasic.INSTANCE;
        ...
        if (m_Dirty.isDirty()) {
            if (m_iState == ST_UPDATE) {
                ...
                if (isCustomerChangeEvent) {
                    ...
                    boolean confirmacion = AlertasPersonalizadas.mostrarAlertaConfirmacion(LocalRes.getIntString("message.customerassign"));
    ...
    public void actionReloadCurrent(Component c) {        
        if (!m_Dirty.isDirty() || AlertasPersonalizadas.mostrarAlertaConfirmacion(LocalRes.getIntString("message.changeslost")))
                {  
            refreshCurrent();
        }             
    }
